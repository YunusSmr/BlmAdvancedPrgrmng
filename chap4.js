...
function analyze(min = 0) {
    return [...EVENTS]
        ...
        .map(x => x.evt + ": " + x.cor.toFixed(4) + " " + (tableFor(x.evt)[1] + tableFor(x.evt)[3]));
...
JOURNAL.map(e => e.squirrel);
JOURNAL.forEach(e => console.log(e.events.toString()));
JOURNAL.filter(e => e.squirrel).map(e => e.events).reduce((a, b) => a.concat(b)).filter((e, i, a) => a.indexOf(e) == i).sort().forEach(e => console.log(e));
